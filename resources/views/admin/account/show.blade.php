        ...
        <div class="tw-flex tw-flex-col tw-mt-8">
            ...
            <div class="tw-flex tw-flex-col tw-ml-8 tw-mt-14">
                <div class="tw-flex tw-float-left tw-items-center">
                    <i
                        class="fa-solid fa-user tw-text-center text-9xl tw-bg-sims-new-500 tw-rounded-full tw-drop-shadow-xl tw-pt-10 tw-overflow-hidden tw-text-white tw-w-40 tw-h-40"></i>
                    <div class="tw-ml-8">
                        ...
                        <div class="sims-heading-3xl-black">
                            ...
                            @if ($user->is_admin == 1)
                                <i title="User ini merupakan Admin"
                                    class="fa-solid fa-shield-check tw-text-sims-500 tw-text-2xl tw-ml-1"></i>
                            @endif
                        </div>
                    </div>
                </div>
                ...
                <div class="tw-float-right tw-mr-8 tw-flex tw-flex-col">
                    ...
                    {{-- klo user adalah admin tapi user yg diview bukan akun sendiri --}}
                    @if (auth()->user()->is_admin == 1 && auth()->user()->id !== $user->id)
                        <form action="/admin/account/{{ $user->id }}" method="POST"
                            class="tw-float-right tw-text-right tw-mb-5 -tw-mt-20">
                            @csrf
                            @method('DELETE')

                            <button type="button" data-modal-toggle="popup-modal"
                                class="tw-bg-red-400 hover:tw-bg-red-600 tw-transition-all tw-w-fit tw-px-4 tw-py-2 tw-rounded-xl tw-text-base tw-text-white tw-font-satoshi"><i
                                    class="fa-regular fa-trash-can tw-text-lg tw-mr-2"></i>Hapus Akun</button>
