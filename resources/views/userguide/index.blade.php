@extends('layouts.main-new')

@section('content')

<div class="tw-container tw-mx-auto">
    
    <!-- spacing !-->
    <div class="tw-mt-10"></div> 
    <!-------------->

    <div class="md:shrink-0">
        <img src="{{ URL::asset('assets/img/hc-header.jpg') }}" alt="hc-header-img" class="tw-rounded-xl">
    </div>
    
    <!-- spacing !-->
    <div class="tw-mt-24"></div> 
    <!-------------->

    <div class="tw-mx-20">

        <div class="hc-title-lg"> 
             Butuh Bantuan?
        </div>
        <div class="hc-text-base">
            Jangan khawatir! Lorem ipsum, dolor sit amet consectetur adipisicing elit. 
            In dicta dolore expedita ex totam cupiditate asperiores, 
            aperiam magni delectus excepturi quis distinctio laboriosam 
            ea quidem, aliquam eius? Eveniet in ab fuga placeat, 
            excepturi fugit sit eius nam illo accusantium iusto, 
            harum numquam commodi possimus rerum dolores enim 
            debitis consectetur ipsam sequi laudantium tempora? 
            Suscipit incidunt provident recusandae fugit in esse 
            doloremque neque porro minima! Maxime aut dolor 
            suscipit hic deleniti, dolores excepturi autem. 
            Enim, distinctio aspernatur labore, 
            similique voluptates facilis porro tenetur 
            esse at accusantium laborum architecto odio 
            voluptatibus earum rerum perspiciatis, 
            quas cum aperiam reprehenderit cumque. 
            Enim, nisi quo!
        </div>
        <br>
        <div class="hc-text-base">
            Lorem ipsum dolor sit amet consectetur adipisicing elit.
            Aliquid itaque est quia expedita explicabo quod deserunt,
            ad sunt molestiae sequi at ipsum,
            voluptatum reiciendis doloribus ratione,
            natus neque a porro iure dicta. Animi,
            earum sunt laborum nam quis inventore iusto incidunt
            assumenda repellendus laudantium ipsa dolore sequi exercitationem
            voluptatibus dignissimos facilis! Libero sint delectus
            repudiandae unde nisi ratione officia aliquid illum ex aliquam.
            Soluta illum nam ratione sint beatae laudantium molestias
            possimus est quisquam iusto officia dicta voluptatibus,
            ipsam natus illo facere placeat voluptatum alias reiciendis
            molestiae labore! Voluptatum praesentium placeat quo,
            a mollitia corporis, omnis ipsa veritatis facilis maxime
            dolorem consequatur consectetur voluptatem tempora.
            Cumque aspernatur dolorum magnam tenetur nobis nemo
            corporis facere animi quos voluptates veniam,
            voluptate autem doloremque. Tempore obcaecati
            magnam aspernatur error natus voluptate neque
            explicabo sequi consectetur porro vitae quas
            dignissimos deleniti autem nulla at nostrum
            labore, nisi consequatur? Suscipit exceptur
            natus praesentium provident nam animi vero iure possimus,
            totam, officiis facilis facere quis reprehenderit debitis
            dolorum sapiente sunt quidem sequi. Animi nemo quis
            officiis maiores aperiam fugiat asperiores suscipit
            veniam consectetur sunt, nobis reprehenderit illum
            voluptas labore architecto autem. Amet repudiandae
            veniam quam adipisci repellendus illum, et, nobis natus
            recusandae, sit assumenda laborum provident.
        </div>

        

        <!-- card !--> 
        <div class="tw-flex tw-flex-wrap-reverse tw-justify-center tw-gap-x-5 tw-gap-y-10 tw-my-24">

            <a href="#" class="hc-card">
                <i class="fa-sharp fa-regular fa-book-open tw-text-3xl tw-text-sims-400 tw-mb-2"></i>
                <div class="hc-title-lg">Data Induk</div>
                <div class="hc-text-xs">Lorem ipsum dolor sit amet consectetur adipisicing elit. Tempore, consequatur!</div>
            </a>

            <a href="#" class="hc-card">
                <i class="fa-regular fa-users tw-text-3xl tw-text-sims-400 tw-mb-2"></i>
                <div class="hc-title-lg">Mutasi</div>
                <div class="hc-text-xs">Lorem ipsum dolor sit amet consectetur adipisicing elit. Tempore, consequatur!</div>
            </a>

            <a href="#" class="hc-card">
                <i class="fa-regular fa-book tw-text-3xl tw-text-sims-400 tw-mb-2"></i>
                <div class="hc-title-lg">Rekap Nilai</div>
                <div class="hc-text-xs">Lorem ipsum, dolor sit amet consectetur adipisicing elit</div>
            </a>

            <a href="#" class="hc-card">
                <i class="fa-regular fa-user tw-text-3xl tw-text-sims-400 tw-mb-2"></i>
                <div class="hc-title-lg">Akun</div>
                <div class="hc-text-xs">Lorem ipsum dolor sit amet consectetur adipisicing elit. Tempore, consequatur!</div>
            </a>

            <a href="/help/general" class="hc-card">
                <i class="fa-solid fa-question tw-text-3xl tw-text-sims-400 tw-mb-2"></i>
                <div class="hc-title-lg">SIMS</div>
                <div class="hc-text-xs">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Aut, nemo?</div>
            </a>

        </div>

        <div class="hc-title-lg"> 
            Lorem Ipsum dolor sit Amet
        </div>
        <div class="hc-text-base">
            Lorem ipsum dolor sit amet consectetur adipisicing elit.
            Aliquid itaque est quia expedita explicabo quod deserunt,
            ad sunt molestiae sequi at ipsum,
            voluptatum reiciendis doloribus ratione,
            natus neque a porro iure dicta. Animi,
            earum sunt laborum nam quis inventore iusto incidunt
            assumenda repellendus laudantium ipsa dolore sequi exercitationem
            voluptatibus dignissimos facilis! Libero sint delectus
            repudiandae unde nisi ratione officia aliquid illum ex aliquam.
            Soluta illum nam ratione sint beatae laudantium molestias
            possimus est quisquam iusto officia dicta voluptatibus,
            ipsam natus illo facere placeat voluptatum alias reiciendis
            molestiae labore! Voluptatum praesentium placeat quo,
            a mollitia corporis, omnis ipsa veritatis facilis maxime
            dolorem consequatur consectetur voluptatem tempora.
            Cumque aspernatur dolorum magnam tenetur nobis nemo
            corporis facere animi quos voluptates veniam,
            voluptate autem doloremque. Tempore obcaecati
            magnam aspernatur error natus voluptate neque
            explicabo sequi consectetur porro vitae quas
            dignissimos deleniti autem nulla at nostrum
            labore, nisi consequatur? Suscipit exceptur
            natus praesentium provident nam animi vero iure possimus,
            totam, officiis facilis facere quis reprehenderit debitis
            dolorum sapiente sunt quidem sequi. Animi nemo quis
            officiis maiores aperiam fugiat asperiores suscipit
            veniam consectetur sunt, nobis reprehenderit illum
            voluptas labore architecto autem. Amet repudiandae
            veniam quam adipisci repellendus illum, et, nobis natus
            recusandae, sit assumenda laborum provident.
        </div>


        <!-- spacing !-->
        <div class="tw-mt-20"></div> 
        <!-------------->
    </div>

</div>

@endsection