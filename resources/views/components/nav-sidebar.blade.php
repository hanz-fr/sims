<div class="tw-flex" x-data="{ open: false }">
    {{-- sidebar --}}
    <aside x-bind:class="open?' tw-left-0 tw-h-screen tw-sticky tw-w-60 tw-bg-white @if(Route::current()->getName() === 'history') tw-shadow-xl @endif tw-flex tw-flex-col tw-top-0 tw-bottom-0' : 'tw-sticky tw-inset-0 tw-w-16 tw-bg-white tw-flex tw-flex-col tw-top-0 tw-bottom-0 tw-h-screen'" class="tw-transition-all tw-duration-300 @if(Route::current()->getName() === 'history') tw-shadow-xl @endif" x-transition:enter.duration.500ms x-transition:leave.duration.400ms>
