...
<body style="background-image: url('{{ URL::asset('assets/img/bg-login.svg') }}')">
    ...
    {{-- @if ($status == 'success')
        <div class="tw-mx-auto tw-container">
            <section
                class="tw-bg-white tw-font-satoshi tw-rounded-[50px] tw-flex tw-flex-col tw-mx-auto tw-my-28 tw-px-20 tw-py-11 tw-top-1/2 tw-w-3/5 tw-h-3/5 tw-border tw-border-slate-200 tw-shadow-xl tw-text-center">
                <img class="tw-w-1/2 tw-mx-auto" src="{{ URL::asset('assets/img/regist-sc.svg') }}"
                    alt="Registration Success">
                <div class="tw-text-3xl tw-font-bold tw-text-sims-new-500 tw-mt-5">Registrasi Berhasil</div>
                <div class="tw-text-sm tw-text-basic-300 tw-font-medium tw-mt-3">Silahkan masuk dengan akun baru anda.
                </div>
                <a href="/login">
                    <button
    ...
    @else --}}
        <div class="tw-mx-auto tw-flex tw-justify-center tw-items-center tw-h-screen tw-container">
            ...
            <!-- card -->
            <div
                class="tw-flex lg:tw-flex-row tw-h-fit sm:tw-flex-col-reverse tw-bg-white tw-px-20 tw-py-16 tw-mx-auto tw-w-2/3 tw-border tw-border-slate-200 tw-shadow-xl">

                <!-- form section -->
                <section class="lg:tw-w-3/5 lg:tw-mr-8 sm:tw-w-full">
                    <div class="sims-heading-3xl md:tw-mt-3 sm:tw-text-center lg:tw-text-left">Selamat Datang</div>
                    <div class="tw-text-sm tw-font-sg tw-mt-2 tw-text-gray-400 sm:tw-text-center lg:tw-text-left">Masuk
                        untuk mengakses website</div>

                    <form action="/loginuser" method="post" class="tw-mt-12">
                        @csrf
                        <ul class="tw-flex tw-flex-col tw-gap-5 tw-font-sg">
                            <li>
                                <input type="text" name="nip" @error('nip') is-invalid @enderror
                                    placeholder="NIP" class="login-input" required>
                                @error('nip')
                            ...
                            <li x-data="{ show: true }">
                                <div class="tw-relative tw-w-full">
                                    <input name="password" @error('password') is-invalid @enderror
                                        placeholder="Kata Sandi" :type="show ? 'password' : 'text'"
                                        class="login-input tw-w-full" required>
                                    <div
                                        class="tw-absolute tw-inset-y-0 tw-right-0 tw-pr-3 tw-flex tw-items-center tw-text-sm tw-leading-5">

                                        <svg class="tw-h-5 tw-text-gray-500" fill="none" @click="show = !show"
                                            :class="{ 'tw-hidden': !show, 'tw-block': show }"
                                        ...
                                        <svg class="tw-h-5 tw-text-gray-500" fill="none" @click="show = !show"
                                            :class="{ 'tw-block': !show, 'tw-hidden': show }"
                                ...
                                <div class="tw-flex">
                                    <div class="tw-flex tw-items-center tw-h-5">
                                        <input id="remember" name="remember" type="checkbox"
                                            class="tw-w-4 tw-h-4 tw-bg-gray-50 tw-rounded tw-border tw-border-gray-500 focus:ring-3 focus:tw-ring-sims-new-500">
                                    </div>
                                    <label for="remember"
                                        class="tw-ml-1 tw-text-sm tw-font-medium tw-text-sims-new-500">Remember
                                        me</label>
                                </div>
                                <a href="/forgot-password" class="tw-text-sims-new-500 tw-underline tw-text-sm">Lupa
                                    Kata Sandi?</a>
                            </li>
                            <li>
                                <button type="submit"
                                    class="tw-bg-[#90C2C2] tw-w-full tw-py-3 tw-text-sm tw-font-medium tw-text-white hover:tw-bg-[#5B9C9C]">Masuk</button>
                            </li>
                ...
                <img class="lg:tw-w-1/2 sm:tw-mb-4 sm:tw-w-full tw-items-center md:tw-m-auto tw-mr-0"
                    src="{{ URL::asset('assets/img/sims-login.svg') }}" alt="" srcset="">
            </div>
        </div> <!-- container -->
    {{-- @endif --}}
