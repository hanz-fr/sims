  ...
  <div class="tw-mx-auto tw-my-14 tw-container">
    ...
    <div class="tw-bg-white tw-flex lg:tw-flex-row sm:tw-flex-col-reverse tw-mx-auto tw-px-20 tw-py-12 tw-w-2/3 tw-h-3/5 tw-border tw-border-slate-200 tw-shadow-xl tw-p-5">
        ...
        <div class="input-area lg:tw-w-3/5 sm:tw-w-full">
          <div class="tw-text-3xl tw-text-sims-400 tw-font-pop tw-font-bold">Registrasi</div>
          <div class="tw-text-sm tw-mt-2 tw-text-slate-400 tw-font-pop">Daftar untuk membuat akun baru</div>
          <form action="/registeruser" method="post" class="tw-mt-10 tw-pr-8">
            @csrf
            <ul class="tw-flex tw-flex-col tw-gap-5">
              <li>
                <input type="text" name="nama" id="nama" @error('nama') is-invalid @enderror placeholder="Nama" class="tw-font-ubuntu tw-font-medium tw-px-4 tw-py-3 tw-border-2 tw-text-gray-500 tw-border-gray-300 tw-w-full tw-block tw-text-sm placeholder:tw-text-gray-400 focus:placeholder:tw-invisible focus:tw-outline-none focus:tw-ring-1 focus:tw-ring-sims-400 focus:tw-border-sims-400 invalid:tw-text-pink-700 tw-peer invalid:focus:tw-ring-pink-700 invalid:focus:tw-border-pink-700">
                @error('nama')
                  <div class="tw-text-sm tw-text-pink-700 tw-mt-1 tw-font-ubuntu">{{ $message }}</div>
                @enderror
              </li>
              <li>
                <input type="text" name="nip" id="nip" @error('nip') is-invalid @enderror placeholder="NIP" maxlength="20" class="tw-font-ubuntu tw-font-medium tw-px-4 tw-py-3 tw-border-2 tw-text-gray-500 tw-border-gray-300 tw-w-full tw-block tw-text-sm placeholder:tw-text-gray-400 focus:placeholder:tw-invisible focus:tw-outline-none focus:tw-ring-1 focus:tw-ring-sims-400 focus:tw-border-sims-400 invalid:tw-text-pink-700 tw-peer invalid:focus:tw-ring-pink-700 invalid:focus:tw-border-pink-700">
                @error('nip')
                  <div class="tw-text-sm tw-text-pink-700 tw-mt-1 tw-font-ubuntu">{{ $message }}</div>
                @enderror
              </li>
              <li>
                <input type="email" name="email" id="email" @error('email') is-invalid @enderror placeholder="Email" class="tw-font-ubuntu tw-font-medium tw-px-4 tw-py-3 tw-border-2 tw-text-gray-500 tw-border-gray-300 tw-w-full tw-block tw-text-sm placeholder:tw-text-gray-400 focus:placeholder:tw-invisible focus:tw-outline-none focus:tw-ring-1 focus:tw-ring-sims-400 focus:tw-border-sims-400 invalid:tw-text-pink-700 tw-peer invalid:focus:tw-ring-pink-700 invalid:focus:tw-border-pink-700">
                @error('email')
                  <div class="tw-text-sm tw-text-pink-700 tw-mt-1 tw-font-ubuntu">{{ $message }}</div>
                @enderror
              </li>
