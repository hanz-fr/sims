      ...
      <div class="tw-flex tw-flex-col tw-my-9">
        ...
        <div class="tw-grid lg:tw-grid-cols-3 md:tw-grid-cols-2 tw-gap-5 tw-mt-8 sm:tw-grid-cols-1">
            ...
            @for ($i = \Carbon\Carbon::now()->year - 5; $i < \Carbon\Carbon::now()->year; $i++)
                @while ($i != $n)
                    <a
                        href="/data-alumni?jurusan={{ $_GET['jurusan'] }}&angkatan={{ $i++ }}&page=1&perPage=10&search=">
                        <div
                            class="tw-bg-white tw-shadow-md hover:tw-bg-sims-new-500 hover:tw-text-white tw-text-sims-new-500 tw-p-5 tw-rounded-xl tw-transition-all tw-delay-[100] hover:-tw-translate-y-1 hover:tw-shadow-lg tw-ease-in">
